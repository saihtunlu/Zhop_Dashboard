  ...
  <div class="row vs-con-loading__container" id="loading">
    ...
    <div class="col-lg-12 padding open div_shadow">
      <div class="row">
        <div class="col-12">
          <div class="flex-between">
            <h4 class="header-text flex-align-center" style="margin-bottom:0.7rem !important;">
              <vs-icon size="small" class="custom_icon" icon="mood"></vs-icon>Edit Product
            </h4>
          </div>
        ...
        <div class="col-12 edit1 mb-3 productImage">
          <div class="con-upload">
            <div class="con-img-upload">
              ...
              <div class="img-upload" v-for="(image, n) in newImages" :key="n">
        ...
        <div class="col-lg-12 mb-3">
          <vs-textarea label="Short Description" v-model="product.short_description" />
        </div>
        <div class="col-lg-12 mb-3">
          <label
            for
            ...
            <div class="col-12 edit1 mb-3 productImage">
              <div class="con-upload">
                <div class="con-img-upload">
                  ...
                  <div class="img-upload" v-if="variation.image">
                    ...
                    <img :src="variation.image" width="100%" />
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    getProduct(id) {
      axios
        .get(`/web/getProductDetail/${id}`)
        .then((response) => {
          this.product = response.data;
          console.log("getProduct -> this.product", this.product);
          this.newImages = this.product.images;
          ...
          this.product.tags.forEach((data) => {
            this.product.newTags.push(data.tag.name);
          });

          if (this.product.type === "Variable Product") {
            this.getAttr(this.product.id);
    ...
    clear_file(n) {
      this.newImages.splice(n, 1);
    },
    clear_fileVari(n) {
      this.variations[n].image = null;
    ...
    onFileChange(e) {
      ...
      files.forEach((data) => {
        array.file = data;
        array.path = URL.createObjectURL(data);
        this.newImages.push(array);
        array = {};
      });
    },
    onFileChangeVari(e) {
      ...
      reader.onload = (e) => {
        this.variations[this.vari_index].image = e.target.result;
      };
    ...
    save() {
      ...
      if (this.newImages[0]) {
        formData.append("file1", this.newImages[0].file);
      }
      if (this.newImages[1]) {
        formData.append("file2", this.newImages[1].file);
      }
      if (this.newImages[2]) {
        formData.append("file3", this.newImages[2].file);
      }
      if (this.newImages[3]) {
        formData.append("file4", this.newImages[3].file);
      }
      if (this.newImages[4]) {
        formData.append("file5", this.newImages[4].file);
      }
      if (this.newImages[5]) {
        formData.append("file6", this.newImages[5].file);
      }
      if (this.newImages[6]) {
        formData.append("file7", this.newImages[6].file);
      }
      if (this.newImages[7]) {
        formData.append("file8", this.newImages[7].file);
      }
      if (this.newImages[8]) {
        formData.append("file9", this.newImages[8].file);
      }
      if (this.newImages[9]) {
        formData.append("file10", this.newImages[9].file);
      }
      if (this.newImages[10]) {
        formData.append("file11", this.newImages[10].file);
      }

      axios
        .post("web/updateProduct", formData, config)
        .then((response) => {
