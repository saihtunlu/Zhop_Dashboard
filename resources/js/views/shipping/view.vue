  ...
  <div class="row vs-con-loading__container" id="loading">
    ...
    <nav aria-label="breadcrumb" class="vs-breadcrumb vs-align-left">
      <ol class="vs-breadcrumb--ol">
        <h3 class="BC_header">Shipping</h3>
        <li class="BC_link">
        ...
        <li class="vs-active BC_link">
          <a class="vs-breadcrumb--link">{{shipping.title}}</a>
        </li>
      </ol>
    </nav>
    <!--End of breadcrumb -->
      ...
      <div class="row">
        ...
        <div class="col-lg-12 mb-3">
          <div style="animation:open 0.3s;" class="custom_input multiShow">
            ...
            <el-select v-model="newCities" multiple filterable default-first-option>
              <el-option
...
export default {
  ...
  methods: {
    getShipping(id) {
      axios
        .get(`web/getShipping/${id}`)
        .then((response) => {
          this.shipping = response.data;
          var array = [];
          this.shipping.cities.forEach((data) => {
            array.push(data.value);
          });
          this.newCities = array;
        })
        .catch((error) => {
          console.log("getShipping -> error", error);
        });
    },
    addMethod() {
      var array = {};
